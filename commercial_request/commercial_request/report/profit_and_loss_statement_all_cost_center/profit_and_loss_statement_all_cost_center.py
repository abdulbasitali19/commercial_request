...
def get_data(filters, dimension_list):
	...
	acc = frappe.db.sql(
		...
		where
			...
			order by lft""",
		(filters.company, filters.root_type),
		as_dict=True,
	)
	...
	min_lft, max_rgt = frappe.db.sql(
		...
		where company=%s and root_type=%s""" ,
		(filters.company, filters.root_type),
	)[0]

	account = frappe.db.sql_list(
		"""select name from `tabAccount`
		where lft >= %s and rgt <= %s and company = %s and root_type=%s""" ,
		(min_lft, max_rgt, filters.company, filters.root_type),
	)
